...
1. Clone the latest `cicd/workflow.sacl-policy.yml`, rename `workflow.sacl-policy.yml` to `.sacl-policy.yml`, drop this in your Root/Defined repo directory.
...
**Grant of License does not include rights to use these trademarks for commercial certification.** No entity may issue, display, or sell "SMCF Certified", "SACL Certified", "SACL Approved", or "Synapticide Compliant" designations without express written authorization and formal certification from the IP holder. See **TRADEMARKS.md** for full usage terms, trademark and copyrights.
